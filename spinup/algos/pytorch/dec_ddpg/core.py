...
class MLPActor(nn.Module):
    ...
    def forward(self, obs):
        ...
        # return self.pi(obs)*torch.Tensor(self.act_limit)
...
class MLPActorCritic(nn.Module):
    ...
    def act(self, obs):
        with torch.no_grad():
            ...
            # print([self.pi(obs)*a for a in self.act_limit])
            # return [self.pi(obs)*a for a in self.act_limit][0].numpy()
...
class DDPG:
    def __init__(self, ac, pi_lr, q_lr, gamma, polyak, logger):
        ...
        self.q_optimizer = Adam(ac.q.parameters(), lr=q_lr)
        self.gamma = gamma
        self.polyak = polyak
        self.logger = logger
    ...
    def get_action(self, o, noise_scale):
        ...
        a += noise_scale * np.random.randn(self.ac.act_dim)
        return np.clip(a, -self.ac.act_limit, self.ac.act_limit)